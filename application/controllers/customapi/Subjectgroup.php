...
class Subjectgroup extends MY_Controller
{
    ...
    public function getSubjectByClassandSectionDate()
    {
        ...
        $date       = date('Y-m-d', $this->customlib->datetostrtotime($this->input->post('date',true)));
       ...
       // echo json_encode($data);
         return $this->output->set_output(json_encode([
                        'status' => true,
                        'success_message' => "getSubjectByClassandSectionDate List",
                        'data' => $data
                    ]));
