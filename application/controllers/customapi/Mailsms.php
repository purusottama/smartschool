...
class Mailsms extends MY_Controller
{
    ...
    public function index()
    {
        ...
        return $this->output->set_output(json_encode([
                        'status' => true,
                        'success_message' => "Notification List",
                        'data' => $data
                    ]));
    }
        ...
        if ($keyword != "" and $category != "") {
            if ($category == "student") {
                ...
                foreach ($result as $key => $value) {
                    $result[$key]['mobileno'] = $value['mobileno'] ?  $value['mobileno']: $value['guardian_phone'];
                    $result[$key]['STUDENTNAME'] = $value['firstname'];
                    $result[$key]['PARENTNAME'] = $value['guardian_name'];
                ...
                $result = $this->student_model->searchNameLike($keyword);
                foreach ($result as $key => $value) {
                    $result[$key]['mobileno']= $value['guardian_phone'] ? $value['guardian_phone'] : $value['mobileno'];
                    $result[$key]['STUDENTNAME'] = $value['firstname'];
                    $result[$key]['PARENTNAME'] = $value['guardian_name'];
                ...
                //  $result[$key]['mobileno']= $value['guardian_phone'] ?? $value['mobileno'];
                // //$result[$key]['STUDENTNAME'] = $value['firstname'];
                //  //$result[$key]['PARENTNAME'] = $value['guardian_name'];
                
                // $result['STUDENTNAME'] = $result['firstname'];
                //  $result['PARENTNAME'] = $result['guardian_name'];
                // //print_r($value);
                // print "<pre>";
                // print_r($result);
            ...
            foreach ($userlisting as $userlisting_key => $userlisting_value) {
                $array = array(
                    ...
                    'mobileno'      => $userlisting_value[0]->mobileno,
                     'PARENTNAME'     => $userlisting_value[0]->guardian_name,//$userlisting_value['guardian_name'],
                    'STUDENTNAME'     => $userlisting_value[0]->firstname,//$userlisting_value['firstname'],
                ...
                if ($send_sms) {
                    ...
                    foreach ($user_array as $user_mail_key => $user_mail_value) {
                        if ($user_mail_value['mobileno'] != "") {
                             $message =  $this->parseTemplate($tempmessage, $user_mail_value);
                    ...
                    if (!empty($userdata_array)) {
                        foreach ($userdata_array as $student_key => $student_value) {
                            ...
                            $array = array(
                                ...
                                'STUDENTNAME'     => $student_value['firstname'],
                                'PARENTNAME'    => $student_value['guardian_name'],
                               'role' => 'student'
                            );
                            $user_array[] = $array;
                        }
                    ...
                    if (!empty($userdata_array)) {
                        foreach ($userdata_array as $parent_key => $parent_value) {
                            $array = array(
                                ...
                                'STUDENTNAME'     => $parent_value['firstname'],
                                'PARENTNAME'    => $parent_value['guardian_name'],
                                'role' => 'parent'
                            );
                            $user_array[] = $array;
                        }
                    ...
                    foreach ($user_array as $user_mail_key => $user_mail_value) {
                        if (in_array("sms", $sms_mail)) {
                            if ($user_mail_value['mobileno'] != "") {
                               ...
                               $message =  $this->parseTemplate($tempmessage, $user_mail_value,$variables);
    ...
    public function parseTemplate(string $message, array $user_mail_value = [], array $variables = []): string
{
    return preg_replace_callback('/\{\{\s*(.*?)\s*\}\}/', function ($matches) use ($variables, $user_mail_value) {

        $key = trim($matches[1]);

        // 1️⃣ User input has priority
        if (isset($variables[$key]) && $variables[$key] !== '') {
            return $variables[$key];
        }

        // 2️⃣ Fallback to system values
        if (isset($user_mail_value[$key])) {
            return $user_mail_value[$key];
        }

        // 3️⃣ Leave placeholder if nothing found
        return $matches[0];
    ...
    function parseTemplateBC($template, $data)
{
    return preg_replace_callback('/{{(.*?)}}/', function ($m) use ($data) {
        ...
        // If key NOT exists → return key without {{}}
        return $key;
    }, $template);
}
                ...
                foreach ($user_array as $user_mail_key => $user_mail_value) {
                    if (in_array("sms", $sms_mail)) {
                        if ($user_mail_value['mobileno'] != "" && $user_mail_value['mobileno'] != 0) {
                            // $message =  $this->parseTemplate($message, $user_mail_value);
                            $this->smsgateway->sendSMS($user_mail_value['mobileno'], ($message), $template_id, "");
                        }
                    }
                }
        ...
        if ($this->form_validation->run()) {
            ...
            foreach ($userlisting as $userlisting_key => $userlisting_value) {
                $array = array(
                    ...
                    'guardianEmail' => $userlisting_value[0]->guardianEmail,
                    'mobileno'      => $userlisting_value[0]->mobileno,
                    'STUDENTNAME'      => $userlisting_value[0]->STUDENTNAME,
                    'PARENTNAME'      => $userlisting_value[0]->PARENTNAME,
                    ...
                    foreach ($section as $section_key => $section_value) {
                        ...
                        if (!empty($userlisting)) {
                            foreach ($userlisting as $userlisting_key => $userlisting_value) {
                                ...
                                'PARENTNAME'     => $userlisting_value['guardian_name'],
                                'STUDENTNAME'     => $userlisting_value['firstname'],
                                'role'     => 'student',
                            );
                            $user_array[] = $array;
                            }
   ...
   public function send_class(){
    ...
    private function extractTemplateVariables(string $message): array
{
    ...
    public function smstemplatedata()
    {
        ...
        //print_r($sms_template_list);
        $variablelist = $this->extractTemplateVariables($sms_template_list->message);
        $option = array();
        $select = array();
        foreach($variablelist as $k => $v){
            $select[$v] = '';
        }
        if(in_array('VEHICLE',$variablelist)){
            $option['VEHICLE'] = $this->vehicle_model->onlyVehicleNumber();
        }
        if(in_array('SCHOOL',$variablelist)){
            $select['SCHOOL'] = $this->setting_model->getSetting()->name;
        }
        echo json_encode(array('data' => $sms_template_list,"datetime"=> date('Y-m-d H:i:s'),"variablelist"=>$variablelist,'option'=>$option,'select'=>$select));
    }
