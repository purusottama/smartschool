        ...
        if (!empty($m->data)) {
            foreach ($m->data as $key => $value) {
                ...
                if ($value->is_returned == 1) {
                    $is_return = "Not returned";//"<span class='label label-danger font-weight-normal item_remove' data-item='  $value->id  ' data-category='  $value->item_category  ' data-item_name='  $value->item_name  ' data-quantity='  $value->quantity  ' data-toggle='modal' data-target='#confirm-delete'>" . $this->lang->line('click_to_return') . "</span>";
                } else {
                    $is_return =  "Returned";//" <span class='label label-success'>" . $this->lang->line('returned') . "</span>";
                }
