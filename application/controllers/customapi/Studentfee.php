...
include(FCPATH . 'Razorpay/vendor/autoload.php');
...
class Studentfee extends My_Controller
{
    ...
    public function addfeeRazorpay()
{
    ...
    try {
        ...
        $link_id     = $this->input->get('razorpay_payment_link_id');
        $signature   = $this->input->get('razorpay_signature');
        ...
        if ($txn->payment_status == 'authorized') {
            throw new Exception("Payment already processed");
        }
        $student_session_id = $txn->student_id;

         
//  http://localhost/school-erp/studentfee/addfeeRazorpay?razorpay_payment_id=pay_SLAEvbl2QfyksZ&razorpay_payment_link_id=plink_SLADcNUAcAu9Mx&razorpay_payment_link_reference_id=&razorpay_payment_link_status=paid&razorpay_signature=0d14280c43a2860d0d1c0321d8eae4891814bc323cbb6295882091a9fa1732e6
        // ----------------------------
        // Verify Razorpay payment
        // ----------------------------
        $api = new Api("rzp_test_SKMvNK08db9afO","IXscjRYQ7NHbvmS5SwZ3J68z");
        ...
        // Restore stored POST
        // ----------------------------
        $post = json_decode($txn->collect_fee_json, true);

        if (!$post) {
            throw new Exception("Invalid stored data");
        }

        $_POST = $post;
        
        if($txn->collection_type == 2){ // group collection
        ...
        $collected_array = array();
            ...
            // Staff details
            // ----------------------------
            $staff_record = $this->staff_model
                ->get($this->customlib->getStaffID());

            $collected_by = $this->customlib->getAdminSessionUserName()
                ."(".$staff_record['employee_id'].")";

            $discounts = isset($post['discounts']) ? $post['discounts'] : [];

            $json_array = [
                'amount'          => convertCurrencyFormatToBaseAmount($post['amount']),
                'amount_discount' => convertCurrencyFormatToBaseAmount($post['amount_discount']),
                'amount_fine'     => convertCurrencyFormatToBaseAmount($post['amount_fine']),
            ...
            $data = [
                ...
                'student_fees_master_id' => $post['student_fees_master_id'],
                'fee_groups_feetype_id'  => $post['fee_groups_feetype_id'],
                'amount_detail'          => $json_array,
            ];
            ...
            // Deposit fee
            // ----------------------------
            $inserted_id = $this->studentfeemaster_model->fee_deposit(
                $data,
                $post['guardian_phone'],
                $discounts,
            ...
            if (!$inserted_id) {
                throw new Exception("Fee deposit failed");
            }
        ...
        // Update transaction
        // ----------------------------
        $this->db->where('id',$txn->id)->update(
        ...
        // Send SMS
        // -----------------------------
        $fee = $this->getstudentfeeOnly($student_session_id);

        $smsamount  = $fee['total_Amount'];
        $smsfeepaid = $fee['total_Deposited_Amount'];
        $smsfeedue  = $fee['final_Balance_Amount'];

        $to = $post['guardian_phone'];

        if ($to) {

            $msg = "Please find your updated fees details. Total fee: $smsamount. Fee paid: $smsfeepaid. Fee Due: $smsfeedue. Have a nice day. Powered by EduFeez.";
            ...
            $url = "http://s2.bulksms7.com/api/mt/SendSMS"
                ...
                . "&text=$message"
                . "&route=4"
                . "&DLTTemplateId=1707176258615230217"
    ...
    {
        //here need to implement razorpay pay 
        //end 
        //here Fee Puru
        ...
        if ($this->form_validation->run() == false) {
            ...
            if($this->input->post('payment_mode') == 'Razorpay'){

                    $keyId     = "rzp_test_SKMvNK08db9afO";//"YOUR_KEY_ID";
                    $keySecret = "IXscjRYQ7NHbvmS5SwZ3J68z";//"YOUR_KEY_SECRET";
                    ...
                    $rinsert = array();
                    $rinsert['user_id'] = $getStaffID = $this->customlib->getStaffID();
                    $rinsert['student_id'] = $student_session_id = $this->input->post('student_session_id');
                    $rinsert['order_id'] = $order_id =  $getStaffID."_".$student_session_id."_".date('Ymdhis');
                    $rinsert['collect_fee_json'] = json_encode($_POST);
                    $rinsert['created_at'] = date('Y-m-d H:i:s');
                    $rinsert['updated_at'] = date('Y-m-d H:i:s');

                        
                        $amount = $this->input->post('amount') + $this->input->post('amount_fine') - $this->input->post('amount_discount');
                        ...
                        $response = $api->paymentLink->create([
                            ...
                            'description' => "Payment for Listing #{$order_id}",
                            'customer' => [
                            ...
                            'callback_url' => base_url().'studentfee/addfeeRazorpay',
                            'callback_method' => 'get'
                        ]);
                        ...
                        $rinsert['razorpay_order_id'] = $responseArray['id'];
                        $rinsert['description'] = json_encode($responseArray);
                        $rinsert['amount'] = $amount;

                        $this->db->insert('razorpay_transactions', $rinsert);
                       
                        echo json_encode([
                            'status' => "success",
                            //'paymentId' => $conn->insert_id,
                            'razorpayLink' => $response['short_url'],
          ...
          //Here Purusottama Send message
            ...
            $fee = $this->getstudentfeeOnly($student_session_id);
                ...
                if($to){
                    ...
                    $to = (int)$to;
                  $message = urlencode($msg);
                  $userSMS = "edufiz@1";//$sms_detail->username;
                  $passwordSMS = "edufiz@1";//$sms_detail->password;
                $template_id = "1707176258615230217";
                $url = "http://s2.bulksms7.com/api/mt/SendSMS"
                    . "?user=$userSMS"
    ...
    public function addfeegrp()
    {
        ...
        if ($this->form_validation->run() == false) {
            ...
            if($this->input->post('payment_mode_fee') == 'Razorpay'){
                    ...
                    $rinsert = array();
                    $rinsert['collection_type'] = 2;// 2 group
                    $rinsert['user_id'] = $getStaffID = $this->customlib->getStaffID();
                    $rinsert['student_id'] = $student_session_id = $this->input->post('student_session_id');
                    $rinsert['order_id'] = $order_id =  $getStaffID."_".$student_session_id."_".date('Ymdhis');
                    $rinsert['collect_fee_json'] = json_encode($_POST);
        ...
        //
            ...
            if ($deposited_fees && is_array($deposited_fees)) {
                ...
                $obj_mail['contact_no']      = $send_to =  $student['guardian_phone'];
                $obj_mail['email']           = $student['email'];
                $obj_mail['parent_app_key']  = $student['parent_app_key'];
                $obj_mail['amount']         = $smsamount  = "(" . implode(',', $amount) . ")";
                ...
                //by puru send custome mesage
//here
                $fee = $this->getstudentfeeOnly($student['id']);

                $smsamount = $fee['total_Amount'];
                $smsfeepaid = $fee['total_Deposited_Amount'];
                $smsfeedue = $fee['final_Balance_Amount'];
                $to     =  $send_to; //9861601016;//
                if($to){
                    $msg = "Please find your updated fees details. Total fee: $smsamount. Fee paid: $smsfeepaid. Fee Due: $smsfeedue. Have a nice day. Powered by EduFeez.";
                    $to = (int)$to;
                  $message = urlencode($msg);
    ...
    private function getstudentfeeOnly($id){
...
if (!empty($transport_fees)) {
    ...
    foreach ($transport_fees as $transport_fee) {
        ...
        if (!empty($transport_fee->amount_detail)) {
            $fee_deposits = json_decode($transport_fee->amount_detail);
        ...
        $feetype_balance = $transport_fee->fees - ($fee_paid + $fee_discount);
        $total_balance_amount += $feetype_balance;
    }
}
$return = array();
$return['total_Amount'] = amountFormat($total_amount) . PHP_EOL;
$return['total_Deposited_Amount'] = amountFormat($total_deposite_amount) . PHP_EOL;
$return['final_Balance_Amount'] = amountFormat($total_balance_amount - $alot_fee_discount) . PHP_EOL;
/* ---------- FINAL OUTPUT ---------- */
return $return;
