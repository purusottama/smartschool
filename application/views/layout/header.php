...
// Get current selected DB from session
$current_db = $CI->session->userdata('selectschool');

// Fallback (first load – domain based)
if (!$current_db) {
    $host = $_SERVER['HTTP_HOST'] ?? '';
    $current_db = $current_db_name = ($host === 'school.aarshilearning.com') ? 'school' : 'default';
}

if ($user_id == 99999999) {
?>
<div class="input-group" >
    <select class="form-control" style="border-radius:15px;" id="set_school_DB" onchange="set_school(this.value)" >
        <option value="" >
            Select School
        </option>
        <option value="starschool" >
           S1
        </option>
        <option value="school">
             S2
    ...
    function set_school(val) {
        if(val){
            var conf = confirm("Are You Sure!");
            if(conf){
                var url = "<?=base_url('/')?>"+ 'admin/dbchange/' + val;
...
<?php } ?>
    ...
    ?>
                                    <div class="currency-icon-list" style="display:none;" data-placement="bottom" data-toggle="tooltip" title="<?php echo $this->lang->line('currency') ?>">
    ...
    ?>
                                    <div class="langdiv" style="display:none;" data-placement="bottom" data-toggle="tooltip" title="<?php echo $this->lang->line('language') ?>"><select class="languageselectpicker" onchange="set_languages(this.value)"  type="text" id="languageSwitcher" >
...
if ($this->module_lib->hasActive('calendar_to_do_list')) {
    if ($this->rbac->hasPrivilege('calendar_to_do_list', 'can_view')) {
        ?>
                                            <li class="dropdown d-none" data-placement="bottom" data-toggle="tooltip" style="display:none" title="<?php echo $this->lang->line('task') ?>">
...
$tasklist = $this->customlib->getincompleteTask($userdata["id"],$userdata["role_id"]);
        foreach ($tasklist as $key => $value) {
            ?>
                                        ...
                                        <ul class="dropdown-menu min-w-full sm-drop-down" style="display:none">
...
if ($this->module_lib->hasActive('chat')) {
    if ($this->rbac->hasPrivilege('chat', 'can_view')) {
        ?>
                                         <li class="cal15 d-sm-none" style="display:none"><a data-placement="bottom" data-toggle="tooltip" title="" href="<?php echo base_url() ?>admin/chat" data-original-title="<?php echo $this->lang->line('chat') ?>" class="todoicon"><i class="fa fa-comment-o"></i></a></li>
                                        <?php
}
    ?>
