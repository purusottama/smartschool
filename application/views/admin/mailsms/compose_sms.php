...
?>
                                            ...
                                            <div class="form-group">
                                                ...
                                                <textarea id="group_msg_text" name="group_message" class="form-control compose-textarea" readonly rows="12"><?php echo set_value('message'); ?></textarea>
...
?>
                                            ...
                                            <div class="form-group">
                                                <label><?php echo $this->lang->line('message'); ?></label><small class="req"> *</small>
                                                <textarea id="individual_msg_text" name="individual_message" readonly class="form-control compose-textarea" rows="12"><?php echo set_value('message'); ?></textarea>
...
?>
                                                    ...
                                                    <input type="text" value="" data-record="" data-email="" data-mobileno="" data-studentname="" data-parentname="" class="form-control" autocomplete="off" name="text" id="search-query">
...
?>                                                
                                            ...
                                            <div class="form-group">
                                                <label><?php echo $this->lang->line('message'); ?></label><small class="req"> *</small>
                                                <textarea id="class_msg_text" name="class_message" class="form-control compose-textarea" readonly rows="12"><?php echo set_value('message'); ?></textarea>
...
?>
                                            ...
                                            <div class="form-group">
                                                <label><?php echo $this->lang->line('message'); ?></label><small class="req"> *</small>
                                                <textarea id="birthday_msg_text" name="birthday_message" class="form-control compose-textarea ckeditor" readonly cols="35" rows="20"><?php echo set_value('message'); ?></textarea>
                                            </div>
                                        </div>
                                        <div class="col-md-4">
                                            <div class="form-group">
...
<b>PARENTNAME</b>,  <b>STUDENTNAME</b>, <b>EXAM</b>, <B>EVENT</B>, <B>SCHOOL</B>, <B>TOTALFEE</B>, <B>PAIDFEE</B>, <B>DUEFEE</B>, <B>VEHICLE</B>, <B>STIME<B>, <B>ETIME<B>, <B>DUEDATE<B>, <b>STARTDATE</b>, <b>ENDDATE</b>, <b>DATE</b><small> if not set consider todate</small> 
                     <!-- Puru End--->
                </div>
            </div>
        </div>
...
<script type="text/javascript">
    ...
    $(document).ready(function () {
        $("#search-query").keyup(function () {
            ...
            $("#search-query").attr('data-studentname', "");
            $("#search-query").attr('data-parentname', "");
            $("#suggesstion-box").hide();
    ...
    $(document).on('click', '.selector-list li', function () {
        ...
        $("#search-query").attr('data-studentname', studentname);
        $("#search-query").attr('data-parentname', parentname);
    ...
    $(document).on('click', '.add-btn', function () {
        ...
        if ($.trim(value) != "") {
            ...
            var studentname = $("#search-query").attr('data-studentname');
            var parentname = $("#search-query").attr('data-parentname');
    ...
    $('#template_id').change(function(){
        ...
        $.ajax({
            ...
            success:function(response){
                //create dynamic input box
           let variables = response?.variablelist || [];
            let $container = $('.dynamicFields');

            let skipVariables = ['STUDENTNAME', 'PARENTNAME'];
            let optionsObject = response?.option || {};
            let selectObject  = response?.select || {};
            ...
            $.each(variables, function (index, name) {
                ...
                if (skipVariables.includes(name)) {
                    return true; // continue
                }

                let field = '';
                let selectedValue = selectObject[name] ?? '';

                // 🚗 VEHICLE → dropdown
                if (name === 'VEHICLE') {

                    let vehicleOptions = optionsObject[name] || [];
                    let optionsHtml = `<option value="">Select Vehicle</option>`;

                    $.each(vehicleOptions, function (i, v) {
                        let isSelected = (v.vehicle_no === selectedValue) ? 'selected' : '';
                        optionsHtml += `
                            <option value="${v.vehicle_no}" ${isSelected}>
                                ${v.vehicle_no}
                            </option>
                        `;
                    });
                    ...
                    field = `
                        <div style="max-width:250px;float:left;padding-right:10px;">
                            <label>${name}</label>
                            <select name="variables[${name}]" class="form-control">
                                ${optionsHtml}
                            </select>
                        </div>
                    `;
                ...
                // 📅 DATE / STARTDATE / ENDDATE → date input
                else if (name === 'DATE' || name === 'STARTDATE' || name === 'ENDDATE') {
            ...
            // $.each(variables, function (index, name) {
            //     let field = `
            //         <div class="" style="max-width:250px;float:left;padding-right:10px;">
            //             <label>${name}</label>
            //             <input 
            //                 type="text"
            //                 name="variables[${name}]"
            ...
            // });
                ...
                $('#group_template_id').val(response.data.template_id);
    ...
    var template_id =  $('#individual_template_id').val();
        ...
        $.ajax({
            ...
            $.each(variables, function (index, name) {
                ...
                $('[name="individual_template_id"]') .val(response.data.template_id);
...
<script type="text/javascript">
    ...
    $.ajax({
        ...
        success:function(response){
            ...
            $('[name="class_template_id"]') .val(response.data.template_id);
...
<script type="text/javascript">
   ...
   $.ajax({
    ...
    success:function(response){
        ...
        $('[name="birthday_template_id"]').val(response.data.template_id);
